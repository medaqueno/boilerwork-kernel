...
final class RedisTable
{
    ...
    private function setTableName(string $tableName)
    {
     ...
     * ], true);
     */
    public function insert(array $data, $overwriteById = false): bool
    {
     ...
     * }
     */
    public function selectAllIterative($pageSize = 100): \Generator
    {
        $startIndex = 0;
        while (true) {
    ...
    private function evaluateCondition($row, $column, $operator, $value)
    {
    ...
    private function compareValues($columnValue, $operator, $value)
    {
        switch ($operator) {
            case '=':
    ...
    public function deleteById(string $id): void
    {
        ...
        $recordIndex = $this->getRecordIndexById($this->tableName, $id);

        // Si el registro existe, eliminarlo
        if ($recordIndex !== null) {
            $this->delete($this->tableName, $recordIndex);
        } else {
